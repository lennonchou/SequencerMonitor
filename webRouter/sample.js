            ...
            excel2json({
                ...
                async.parallel(_.map(results, function (result) {
                    return (function (callback) {
                        // TODO: check the must fields
                        // TODO: specify the error, like missing parameters or encounter duplicates
...
exports.getWaitingToProcess = function (req, res) {
    ...
    async.parallel({
        samples: function (callback) {
            connection.myQuery(helper.constructSelectSQL(columns, 'Patient JOIN Sample ON Patient.id = Sample.patientId LEFT JOIN File ON Sample.sampleNumber = File.sampleNumber',
                [{ name: 'File.status', low: 1 }, { name: 'File.status', high: 1 }]) + ' ORDER BY ? LIMIT ?, ?', [lowBound, highBound, orderBy, (page - 1) * limit, limit])
                .then(function (rows) {
...
exports.getWaitingToConfirm = function (req, res) {
    ...
    async.parallel({
        samples: function (callback) {
            connection.myQuery(helper.constructSelectSQL(columns, 'Patient JOIN Sample ON Patient.id = Sample.patientId LEFT JOIN File ON Sample.sampleNumber = File.sampleNumber',
                [{ name: 'File.status', exact: 1 }]) + ' ORDER BY ? LIMIT ?, ?', [waitingToConfirm, orderBy, (page - 1) * limit, limit])
                .then(function (rows) {
                    samples = _.map(rows, function (sample) {
                        return {
                            patientId: sample.id,
